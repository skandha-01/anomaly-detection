...
# Function to detect anomalies using ARIMA
def detect_anomalies_arima(data, order=(1, 0, 0), threshold=3):
    model = sm.tsa.ARIMA(data, order=order)
    model_fit = model.fit(solver='lbfgs', disp=False)
...
def plot_anomaly_points_arima(df, primary_parameters, secondary_parameters, frequency, order=(1, 0, 0), anomaly_alpha=0.7):
    ...
    for parameter in valid_parameters:
        anomalies = detect_anomalies_arima(df[parameter], order=order)
    ...
    ax_anomalies.set_title('Anomalies Detected (ARIMA)')
...
def select_secondary_parameters(df, primary_indices, all_parameters):
    ...
    # Button to plot the anomaly points using ARIMA
    plot_anomalies_arima_button = tk.Button(root, text="Plot Anomaly Points (ARIMA)", command=lambda: plot_anomaly_points_arima(df, primary_parameters, [secondary_dropdown.get(index) for index in secondary_dropdown.curselection()], frequency, order=(1,0,0)))
    plot_anomalies_arima_button.pack(pady=10)
